/* $Id: model_in_ply.cxx,v 1.1 2005/05/24 07:37:58 xushun Exp $ */
                             ...
                             const int swap_bytes, void *out) 
{
  ...
  case int8:
    ...
    if (c == EOF) 
      ...
      tmp_int8= (signed char *)out;
      *tmp_int8 = (t_int8)c;
    }
    break;
  case uint16:
    tmp_uint16 = (unsigned short *)out;
  ...
  do {
    ...
    if (c == 'p') {
      ...
      if (string_scanf(data, stmp) == 1 && strcmp(stmp, "property") == 0) {
        *prop = (struct ply_prop *)realloc(*prop, (n_prop+1)*sizeof(struct ply_prop));
...
int read_ply_tmesh(struct model **tmesh_ref, struct file_data *data) 
{
  ...
  if (skip_ws_str_scanf(data, stmp) == 1) {
    if (strcmp(stmp, "ply") == 0) {
      if (skip_ws_str_scanf(data, stmp) != 1)
        rcode = MESH_CORRUPTED;
    }
    if (strcmp(stmp, "format") == 0) {
      if (skip_ws_str_scanf(data, stmp) == 1) { 
